...
function ChatView({ index = 0, project }: { index?: number; project?: TChatProject }) {
  ...
  const footerBelow = configuredFooter;
  ...
  const chatFormPlaceholder = localize(
    isLandingPage ? 'com_ting_welcome_placeholder' : 'com_ting_composer_placeholder',
  );
              ...
              <Presentation>
                <TraceSurface conversationId={conversationId}>
                  ...
                  <>
                    <div
                      className={cn(
                        'ting-chat-body flex flex-col',
                        isLandingPage
                          ? 'ting-chat-body--landing flex-1 items-center'
                          : 'ting-chat-body--conversation h-full overflow-y-auto',
                      )}
                    >
                      {content}
                      ...
                      <div
                        className={cn(
                          'ting-composer-dock w-full bg-presentation [view-transition-name:chat-form]',
                          !isLandingPage && 'scrollbar-gutter-spacer',
                          isLandingPage && 'ting-composer-dock--landing',
                        )}
                      >
