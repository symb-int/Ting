...
function MessageParts(props: TMessageProps) {
  ...
  const { requireChatCapability, showCapabilityError } = useTingChatCapabilityGuard(
    `${conversation?.conversationId ?? ''}:${messageId ?? ''}`,
  );
  const handleRegenerateWithCapability = useCallback(() => {
    if (requireChatCapability()) {
      regenerateMessage();
    }
  }, [regenerateMessage, requireChatCapability]);
  const handleContinueWithCapability = useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      if (requireChatCapability()) {
        handleContinue(event);
      }
    },
    [handleContinue, requireChatCapability],
  );
  ...
  return (
    ...
    >
      <div className="ting-message-shell m-auto justify-center">
        <MessageRow
          id={messageId ?? ''}
          icon={<MessageIcon iconData={iconData} assistant={assistant} agent={agent} />}
          label={name}
          ...
          footer={
            <SubRow classes={cn(messageFooterClasses, isCreatedByUser && 'justify-end')}>
              ...
              <HoverButtons
                ...
                regenerate={handleRegenerateWithCapability}
                copyToClipboard={copyToClipboard}
                getCanCopy={getCanCopy}
                handleContinue={handleContinueWithCapability}
