...
let mockCapabilityStatus: 'configured' | 'not_configured' = 'configured';

const TRANSLATIONS: Record<string, string> = {
  com_ting_response_error:
...
describe('ToolCallLimitNotice', () => {
  ...
  it('keeps recovery actions visible when MODEL_NOT_CONFIGURED blocks them', () => {
    const ask = jest.fn();
    mockCapabilityStatus = 'not_configured';
    renderNotice({ ask, isSubmitting: false, latestMessageId: 'response-1' });

    const continueButton = screen.getByRole('button', {
      name: TRANSLATIONS.com_ui_tool_call_limit_continue,
    });
    const answerButton = screen.getByRole('button', {
      name: TRANSLATIONS.com_ui_tool_call_limit_answer,
    });
    ...
    expect(screen.getByRole('alert')).toHaveTextContent(TRANSLATIONS.com_ting_response_error);
  });
