...
export default function EditContentParts({
  ...
  const { requireChatCapability, showCapabilityError } = useTingChatCapabilityGuard(messageId);
  ...
  const updateAndRerun = useCallback(() => {
    if (!editedMessage || rerunRequiresSave || hasBlankEdit || isBusy || !requireChatCapability()) {
      return;
    }
