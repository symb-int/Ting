...
const ChatForm = memo(function ChatForm({
  ...
  const disableInputs = invalidAssistant;
  const { requireChatCapability, showCapabilityError } = useTingChatCapabilityGuard(conversationId);
  ...
  const baseClasses = useMemo(
    () =>
      cn(
        'ting-composer-textarea m-0 w-full resize-none bg-transparent placeholder:text-text-tertiary',
        isCollapsed ? 'max-h-[52px]' : 'max-h-[40dvh]',
      ),
    [isCollapsed],
  );

  /** Keep the native one-request-at-a-time boundary before routing a submission. */
  const submitComposerText = useCallback(
    (data: { text: string }): false | void => {
      if (isSubmitting) {
        return false;
      }
      if (!requireChatCapability()) {
        return false;
      }
    ...
    [answerMode, steering, submitMessage, methods, requireChatCapability, isSubmitting],
  );
  ...
  return (
    <form
      ...
      className="ting-chat-form mx-auto flex w-full flex-row gap-3"
    >
      {/* TING I01 hard scope: this sole composition point does not mount
          ProjectLandingChip, QuoteButton, or PendingQuoteChips. Legacy project
          props and persisted quote atoms therefore cannot expose excluded UI. */}
      <div className="relative flex h-full min-w-0 flex-1 items-stretch md:flex-col">
        ...
        <div className="relative flex w-full flex-col">
          {showCapabilityError && (
            <TingStatus className="ting-composer-status" variant="error" role="alert">
          ...
          <div className={cn('flex w-full items-center', isRTL && 'flex-row-reverse')}>
            ...
            <div
              ...
              className={cn(
                ...
                'ting-composer relative flex w-full flex-grow flex-col overflow-hidden',
            ...
            >
              ...
              <div
                className={cn(
                  'ting-composer-actions @container flex flex-wrap items-center gap-2 px-2 pb-2',
                  isRTL ? 'flex-row-reverse' : 'flex-row',
                )}
              >
