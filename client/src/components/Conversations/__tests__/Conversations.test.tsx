...
describe('Conversations: pinned chats live in PinnedSection', () => {
  ...
  it('renders the TING empty state when the flat case list has no rows', () => {
    renderConversations([], '', true);

    expect(screen.getByTestId('convo-list-empty')).toHaveTextContent('com_ting_no_cases');
  });

  it('renders the TING search-empty state when no flat case matches', () => {
    renderConversations([], 'missing', true);

    expect(screen.getByTestId('convo-list-empty')).toHaveTextContent('com_ting_no_cases_found');
  });
});
