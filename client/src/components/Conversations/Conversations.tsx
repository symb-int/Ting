...
import { TingEmptyState } from '~/ting';
...
interface ConversationsProps {
  ...
  /** TING's case list is a single, always-expanded sequence with no date groups. */
  flatMode?: boolean;
}
    ...
    <CellMeasurer cache={cache} columnIndex={0} key={rowKey} parent={parent} rowIndex={index}>
      {({ registerChild }) => (
        <div
          ...
          style={style}
          className="ting-conversation-list-row px-3"
...
const Conversations: FC<ConversationsProps> = ({
  ...
  const convoHeight = flatMode || isSmallScreen ? 44 : 34;
  ...
  const groupedConversations = useMemo(
    () =>
      flatMode
        ? ([['', filteredConversations]] as ReturnType<typeof groupConversations>)
        : groupConversations(filteredConversations, {
            field: sort.field,
            direction: sort.direction,
            includePinned: isArchivedView,
          }),
    [filteredConversations, flatMode, isArchivedView, sort.direction, sort.field],
  );
  const hasGroupedConversations = useMemo(
    () => groupedConversations.some(([, conversations]) => conversations.length > 0),
  ...
  useEffect(() => {
    if (!isChatsExpanded || isLoading || isSearchLoading || hasGroupedConversations) {
      return;
    }
  ...
  const flattenedItems = useMemo(() => {
    ...
    return items;
  }, [flatMode, groupedConversations, isLoading, isChatsExpanded]);
  ...
  const rowRenderer = useCallback(
    ...
    [cache, flatMode, flattenedItems, moveToTop, toggleNav, activeJobIds, sort.field],
  );
  ...
  const hasUnfilteredRows = flatMode
    ? filteredConversations.length > 0
    : !search.query &&
      filterTags.length === 0 &&
      !isArchivedView &&
  ...
  let emptyLabel: TranslationKeys = flatMode ? 'com_ting_no_cases' : 'com_ui_no_chats';
  if (search.query) {
    emptyLabel = flatMode ? 'com_ting_no_cases_found' : 'com_ui_no_search_results';
  ...
  let body: ReactNode = (
    <div ref={setListNode} className="flex-1">
      <List
        ...
        aria-label={localize(flatMode ? 'com_ting_cases' : 'com_ui_conversations')}
  ...
  if (isSearchLoading) {
    ...
    body = flatMode ? (
      <TingEmptyState className="m-3" data-testid="convo-list-empty" title={localize(emptyLabel)} />
    ) : (
      <div
        className="flex flex-1 flex-col items-center justify-center gap-2 px-6 text-center"
  ...
  return (
    <div
      ref={chatsRegionRef}
      className={cn(
        'relative flex flex-1 flex-col pb-2 text-sm text-text-primary',
        flatMode && 'ting-conversations',
      )}
    >
      {flatMode ? (
        <h2 className="ting-conversations__heading">{localize('com_ting_cases')}</h2>
      ) : (
        <div className="px-3">
          <ChatsHeader
