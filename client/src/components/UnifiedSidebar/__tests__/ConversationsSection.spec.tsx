...
jest.mock('~/components/Conversations', () => {
  ...
  const ConversationsStub = memo(function ConversationsStub({ flatMode }: { flatMode?: boolean }) {
    mockConversationsRender();
    return <div data-testid="conversations-stub" data-flat-mode={flatMode || undefined} />;
  });
...
describe('ConversationsSection TING case list', () => {
  it('renders one flat case list without legacy project or pinned sections', async () => {
    const { getByTestId, queryByTestId, getByRole } = renderSection();
    await settleRenders();

    expect(getByRole('region', { name: 'com_ting_cases' })).toBeInTheDocument();
    expect(getByTestId('conversations-stub')).toHaveAttribute('data-flat-mode', 'true');
...
describe('ConversationsSection scroll surface', () => {
  /** Searching replaces the case rows, so a position kept from the previous
   *  result set would open the new results partway down. */
  it('returns the surface to the top when a search replaces its contents', async () => {
    ...
    const surface = container.querySelector<HTMLElement>('.ting-sidebar__history-scroll');
