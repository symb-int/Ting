...
test('renders login form', () => {
  ...
  expect(getByRole('link', { name: /Create account/i })).toHaveAttribute('href', '/register');
...
test('shows the TING session-expired status from the real redirect marker', () => {
  ...
  expect(status).toHaveTextContent('Please sign in again.');

  window.history.pushState({}, '', '/login');
});

test('keeps the neutral registration status across auth-route refreshes', () => {
  sessionStorage.setItem('ting.registration-complete', 'true');

  const { getByRole } = setup();

  expect(getByRole('status')).toHaveTextContent('You can sign in now.');
  expect(sessionStorage.getItem('ting.registration-complete')).toBe('true');
  sessionStorage.removeItem('ting.registration-complete');
});
