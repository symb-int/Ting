...
test('renders registration form', () => {
  ...
  expect(getByRole('form', { name: /Create account/i })).toBeVisible();
...
test('navigates directly to login with a neutral completion marker after registration', async () => {
  window.history.pushState({}, '', '/register');
  const { mockUseRegisterUserMutation } = setup();
  const calls = mockUseRegisterUserMutation.mock.calls;
  const options = calls[calls.length - 1][0] as { onSuccess?: () => void };

  act(() => options.onSuccess?.());

  await waitFor(() => expect(window.location.pathname).toBe('/login'));
  expect((window.history.state as { usr?: unknown }).usr).toEqual({
...
test('shows error message when registration fails', async () => {
  ...
  await waitFor(() => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent(
      /There was an error attempting to register your account. Please try again./i,
    );
  });
});
