...
function appendClickableTarget(attributes: Record<string, string>) {
  const onClick = jest.fn();
  const button = document.createElement('button');
...
describe('binding resolution helpers', () => {
  ...
  it('resolves bindings only for the TING shortcut scope', () => {
    expect(Array.from(TING_SHORTCUTS)).toEqual([
      'showShortcuts',
      'newChat',
      'focusChat',
      'copyLastResponse',
    ...
    const overrides = Object.fromEntries(
      excluded.map((id, index) => [
        id,
        { mac: `Meta+Alt+${index + 1}`, other: `Control+Alt+${index + 1}` },
      ]),
    );

    for (const id of excluded) {
      expect(TING_SHORTCUTS.has(id)).toBe(false);
      expect(effectiveBinding(id, overrides)).toBeNull();
    }
  });
});
...
describe('global shortcut dispatch', () => {
  ...
  it('leaves an excluded shortcut alone when the focused input claimed it', () => {
    renderHarness();
  ...
  it('does not run or swallow an excluded default shortcut', () => {
    renderHarness();
    const uploadClick = appendClickableTarget({ id: 'attach-file-button' });
  ...
  it('does not run or swallow a persisted binding for an excluded shortcut', () => {
    const overrides = {
      openModelSelector: { mac: 'Alt+M', other: 'Alt+M' },
    };
    renderHarness(undefined, '/c/test-convo', (snapshot) => {
      snapshot.set(store.customShortcuts, overrides);
    });
    const modelSelectorClick = appendClickableTarget({ 'data-testid': 'model-selector-button' });

    const event = dispatchKey({ key: 'm', altKey: true });

    expect(modelSelectorClick).not.toHaveBeenCalled();
    expect(event.defaultPrevented).toBe(false);
  });

  it('ignores an excluded custom collision and keeps the allowed default owner', () => {
    const overrides = {
      uploadFile: { mac: 'Meta+Shift+S', other: 'Control+Shift+S' },
    };
    const { getByTestId } = renderHarness(undefined, '/c/test-convo', (snapshot) => {
      snapshot.set(store.customShortcuts, overrides);
    });
