...
describe('GET /api/config', () => {
  describe('unauthenticated (no req.user)', () => {
    ...
    it('should return minimal payload without authenticated-only fields', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(response.body).not.toHaveProperty('modelSpecs');
      expect(response.body).not.toHaveProperty('chatCapability');
  ...
  describe('authenticated (req.user exists)', () => {
    ...
    it('reports the TING chat capability from the loaded model spec and server credential', async () => {
      mockGetAppConfig.mockResolvedValue({
        ...baseAppConfig,
        modelSpecs: {
          list: [
      ...
      expect(response.body.chatCapability).toEqual({ status: 'not_configured' });

      process.env.OPENAI_API_KEY = 'server-key';
      response = await request(app).get('/api/config');
      expect(response.body.chatCapability).toEqual({ status: 'configured' });
    });
    ...
    it('should include post-login informational fields', async () => {
      ...
      expect(response.body.helpAndFaqURL).toBe('');
